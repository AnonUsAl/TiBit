    ...
    try:
        if os.path.exists(Password_hash_file):
            with open(Password_hash_file, 'r', encoding='utf-8') as f:
                p = f.read().strip()
                return p�L��bf*�^D�*b�ib.sha256(DEFAULT_PASSWORD.encode()).hexdigest()
        else:
...
def password_recover():
    ...
    try:
        if messagebox.askyesno("安全验证", "是否重置密码并解除锁定？", parent=temp_root):
            word = simpledialog.askstring("重置密码", "请输入新密码:", show='*', ��x��_[�X���')
...
def check_password(parent=None, max_attempts=3):
    """弹出密码对话，最多��,(]����J��\J��pts 次。返回 True/False。"""
    ...
    try:
        for attempt in range(max_attempts):
            ans = simpledialog.askstring("密码验证", "请输入密码:", show='*', parent=�����B�@z|2k            if ans is None:
                break
            ...
            if password_error_times >= max_attempts:
                messagebox.showwarning("警告", "密码输入错误次数过多，程序将退��hD�T�A�"r��O��ialog_parent)
                lockon(1)
                password_recover()
                pass
    finally:
...
�ݩ�����I���.destroy()
    ...
    try:
        ...
        messagebox.showerror("程序错误", f"程序运行出错: {e}", parenL�b�t�01Y��q�$z try:
...
class Wallet:
    ...
    @staticmethod
    def sign_transaction(priva��}9�W�~o�	��ion_data):
        """使用私钥对交易数据进行签名（简化）。"""
    ...
    def verify_signature(public_key, signature, transaction_data):
        ...
        if transaction_data.get('sender') == "SYSTEM":
            return True
        returd���S:u����> == 64 and public_key is not None
...
class Blockchain:
    def __init__(self):
        ...
        if not loaded:
            self.mine_block(proof=100, previous_hash=GENESIS_P,����/��t��@�D�ctions=[], miner_address="SYSTEM")
    ...
    @staticmet�����f�y��i�[_chain_valid_static(chain):
        if not chain:
            return False
        ...
        for i in range(1, len(chain)):
            ...
            expected_prev_hash = hashlib.sha256(
                json.dumps(OrderedDict(sorte�Yv�[2�[����J�ms())), sort_keys=True).encode()).hexdigest()
  ...
  ��+Jo��n��:���ctions_string = json.dumps(transactions_to_verify, sort_keys=True).encode()
            ...
            expected_hash = hashlib.sha256(
                json���t�X���T5� �ct(sorted(block_copy.items())), sort_keys=True).encode()).hexdigest()
    ...
    def new_transaction(self, sender, recipient, amount, sender_private_ke�θ@I�<2�>^i�  """
...
�!�1p�r�~u�r != "SYSTEM":
        ...
        transaction_data = {
            'sender': sender,
            'recipient': re(M�&�9*������      'amount': amount,
            'timestamp': time.time(),
        }
    ...
    def mine_block(self, proof,��bISZ1���/�transactions, miner_address):
        """
        创建并添加一个新区块到链上
        """
        block = {
        ...
        received_sigs = {tx.get('signaqJr����8{�[wT� transactions if isinstance(tx, dict)}
...
��mq��{h�{L$(balances[tx['recipient']] = balances.get(tx['recipient'], 0) + tx['amount']
    ...
    def is_chain_valid(self):
        ...
        if genesis.get('previous_hash') != GENESIS_PREV_HASH:
            m�f{�����=�Ṃror("链校验失败", "创世区块的 previous_hash 不正确。", parent=tk._default_root)
            return False
        ...
        for i in range(1, len(self.chain)):
            ...
            if current_block.get('previous_hash') != expected_prev_hash:
                messagebox.showerror("链校验失败", f"链条断裂：区块 {i} 的 previous_hash �2r_i5���0�} 的哈希不符。",
            ...
            if not self.is_valid_proof(last_proof, current_block.get('proof'), transactions_to_verify,
                                       current_difficulty�����������\l    messagebox.showerror("链校验失败",
            ...
            if current_block.get('hash') != expected_hash:
                messagebox.showerror("链校验失败",
                            ��N������#Y�2� {i} 的哈希值不正确。期望 {expected_hash}，但区块中为 {current_block.get('hash')}",
                                     parent=tk._default_root)
                return False
    ...
    def save_chain(self, filename):
        ...
        try:
            ...
            with open(filename, 'w', en�k�z?m����_����as f:
    ...
    def load_chain(self, filename):
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                ...
                self.pending_transactiocoding='utf-8') as f:
                json.dump(data, f, indent=4, ensure_ascii=False)
            return True
        except Exception as e:
            messagebox.showerror("保存失败", f"保存失败: {e}", parent=tk._default_root)
